...
def classify_signal(score_total, confianca=100):
    """
    V7.4 — Classificação adaptativa.
    
    Bronze: 55-74 (Aumentamos range para permitir sinais de alta convicção mesmo com score menor)
    Prata: 75-84
    Ouro: 85-94
    Ouro Supremo: 95+
    """
    ...
    if score_total >= SCORE_OURO_SUPREMO_MIN:
        ...
        # Apenas bloqueia se for realmente muito baixo
        if score_total >= 50:
             result["SINAL_BRONZE"] = True
             result["CLASSIFICACAO_FINAL"] = "BRONZE"
             result["QUALIDADE_SINAL"] = "baixa"
        else:
