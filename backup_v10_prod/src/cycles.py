...
from regime import (
    classify_regime, calc_institutional_score, calc_confidence_multiplier,
    detect_traps, check_breakout, RegimeWatchlist,
)
...
from config import (
    ...
    SCORE_PRATA_MIN, SCORE_BRONZE_MIN,
    REGIME_MULTIPLIER_BULL_LONG, REGIME_MULTIPLIER_BULL_SHORT,
    REGIME_MULTIPLIER_BEAR_LONG, REGIME_MULTIPLIER_BEAR_SHORT,
    REGIME_MULTIPLIER_TRANSICAO, REGIME_MULTIPLIER_LATERAL,
)
...
async def processar_par(symbol, tf_data, risk, diagnostics, adaptive, session):
    ...
    # V8 — Classificar regime institucional
    regime, regime_detalhes = classify_regime(
        op_data["TREND"], op_data["MOMENTUM"], op_data["FLOW"]
    )
    v["REGIME"] = regime
    v.update({f"REGIME_{k.upper()}": v for k, v in regime_detalhes.items()})
    ...
    # V8 — Score institucional dinamico (±15pts)
    inst_pontos, inst_detalhes = calc_institutional_score(
        op_data["TREND"], op_data["FLOW"], op_data["MOMENTUM"], direcao,
    )
    score_total = max(0, min(100, score_total + inst_pontos))
    v["SCORE_INSTITUCIONAL"] = inst_pontos
    v["SCORE_TOTAL"] = score_total

    # V8 — Rompimento EMA200
    breakout_pontos, breakou, falso = check_breakout(
        op_data["TREND"], op_data["FLOW"], op_data["MOMENTUM"], preco, direcao,
    )
    score_total = max(0, min(100, score_total + breakout_pontos))
    v["SCORE_BREAKOUT"] = breakout_pontos
    v["BREAKOUT_CONFIRMADO"] = breakou
    v["BREAKOUT_FALSO"] = falso
    v["SCORE_TOTAL"] = score_total

    # V8 — Deteccao de armadilhas
    trap_pontos, traps = detect_traps(op_data["TREND"], op_data["FLOW"], op_data["MOMENTUM"])
    score_total = max(0, min(100, score_total - trap_pontos))
    v["ARMADILHAS"] = traps
    v["PENALIDADE_ARMADILHAS"] = trap_pontos
    v["SCORE_TOTAL"] = score_total
    ...
    # V8 — Multiplicador de confianca por regime
    regime_mult = calc_confidence_multiplier(regime, direcao)
    confianca = int(confianca * regime_mult)
    confianca = max(0, min(100, confianca))
    v["REGIME_MULTIPLICADOR"] = regime_mult
    v["NIVEL_CONFIANCA"] = confianca
