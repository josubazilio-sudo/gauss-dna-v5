...
def _fluxo_desc(flow_data, direction, fluxo_data=None):
    if fluxo_data and isinstance(fluxo_data, dict):
        classificacao = fluxo_data.get("fluxo_classificacao", "")
        if classificacao:
    ...
    if direction == "long" and delta > 0:
        if vol_crescente and rvol > 2.5:
            return "💚 Muito Forte"
        if vol_crescente and rvol > 1.5:
            return "🟢🟢 Forte"
        return "🟢 Moderado"
...
async def send_signal(session, symbol, direction, preco, score,
                      ...
                      flow_data, stop_loss, stop_pct, tp1,
                      timeframe,
                      setup=None,
    ...
    # Probabilidade unificada: conviction_score como medida principal
    ...
    if conviction_score is not None:
        ...
        elif conviction_score >= 50: conviction_label = "Razoável"
    else:
        conviction_score = 0
    ...
    trailing_label = "→ TP1 fecha 50%, saldo vai p/ BE, restante trailing até TP Final"
    ...
    if diagnostico:
        ...
        nivel_risco = diagnostico.get("nivel_risco", "Médio")
        risco_emoji = {"Baixo": "🟢", "Médio": "🟡", "Médio-Alto": "🟠", "Alto": "🔴"}.get(nivel_risco, "⚪")
        if pontos_fortes:
            diag_lines.append(f"✅ Pontos Fortes: {', '.join(pontos_fortes[:3])}")
        if pontos_fracos:
            diag_lines.append(f"⚠️ Pontos Fracos: {', '.join(pontos_fracos[:3])}")
        diag_lines.append(f"{risco_emoji} Nível de Risco: {nivel_risco}")
    ...
    lines = [
        ...
        f"💎 Convicção: {prob}/100 ({conviction_label})",
        "",
