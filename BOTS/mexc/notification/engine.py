...
class NotificationEngine:
    ...
    def notify_trade_opened(self, position: Position) -> None:
        msg = f"🚀 NOVA OPERAÇÃO INICIADA: {position.side.value.upper()} {position.pair} @ {position.entry_price:.2f} (SL: {position.stop_loss:.2f}, TP1: {position.take_profit_1:.2f}, TP2: {position.take_profit_2:.2f})"
    ...
    def notify_tp1_hit(self, position: Position, price: float) -> None:
        msg = f"🎯 TP1 ALCANÇADO: {position.pair} @ {price:.2f}. Parcial executada!"
    ...
    def notify_tp2_hit(self, position: Position, price: float) -> None:
        msg = f"🏆 TP2 ALCANÇADO (ALVO FINAL): {position.pair} @ {price:.2f}. Operação encerrada com lucro!"
    ...
    def notify_stop_hit(self, position: Position, price: float) -> None:
        msg = f"🛑 STOP LOSS ACIONADO: {position.pair} @ {price:.2f}. Operação encerrada."
    ...
    def notify_regime_change(self, pair: str, old_regime: str, new_regime: str) -> None:
        msg = f"🔄 MUDANÇA DE REGIME EM {pair}: {old_regime.upper()} ➡️ {new_regime.upper()}"
    ...
    def notify_daily_report(self, stats: DailyStats) -> None:
        msg = (
            f"📊 RELATÓRIO DIÁRIO - {stats.date}\n"
            f"---------------------------------\n"
            f"Operações Totais: {stats.total_trades}\n"
            f"Vitórias: {stats.winning_trades} | Derrotas: {stats.losing_trades}\n"
            f"Taxa de Acerto: {stats.winning_trades / max(stats.total_trades, 1):.1%}\n"
            f"Resultado Líquido: {stats.net_pnl:+.2f} USDT\n"
            f"Drawdown Máximo: {stats.max_drawdown:.2%}"
        )
