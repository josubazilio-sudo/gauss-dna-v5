...
def build_advanced_report(report) -> Dict[str, Any]:
    ...
    report.decisions ja populado por main.py) e devolve os 11 blocos.
    Somente leitura — nenhum campo de Signal/SignalDecision/ScannerScore
    e alterado ou recalculado.

    RFC V6.7: adiciona blocos resumo_ciclo e mercado_estatisticas;
    se decisions estiver vazia, retorna estrutura minima."""
    decisions = report.decisions

    # RFC V6.7 item 5: guarda para decisions vazia
    if not decisions:
        return {
            "resumo_ciclo": {"mensagem": "Sem ativos suficientes para gerar estatisticas do ciclo."},
            "resumo_scanner": build_scanner_summary(report),
